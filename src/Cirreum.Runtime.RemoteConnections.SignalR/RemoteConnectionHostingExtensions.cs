...
/// <summary>
/// Registration of SignalR remote connections on a Cirreum application builder.
/// </summary>
public static class RemoteConnectionHostingExtensions {
	...
	extension(IDomainApplicationBuilder builder) {
		...
		/// An optional delegate applied to the underlying <see cref="IHubConnectionBuilder"/>
		...
		public IDomainApplicationBuilder AddRemoteConnection<TConnection>(
			...
			builder.Services
				.Add(new ServiceDescriptor(typeof(TConnection), sp => {
					var context = SignalRRemoteConnectionContext.Create<TConnection>(sp, options, configureTransport);
		...
		public IDomainApplicationBuilder AddRemoteConnectionFactory<TConnection>(
			...
			builder.Services
				.Add(new ServiceDescriptor(typeof(IRemoteConnectionFactory<TConnection>),
					sp => new SignalRRemoteConnectionFactory<TConnection>(sp, options, configureTransport),
