...
public class RemoteConnectionRegistrationTests {
	...
	private sealed class TestConnection(SignalRRemoteConnectionContext context) : SignalRRemoteConnection(context) {

		public Uri Endpoint { get; } = context.Options.EndpointUri;

	}

	private sealed class OtherConnection(SignalRRemoteConnectionContext context) : SignalRRemoteConnection(context);

	private const string HubUri = "https://api.example.com/hubs/chat";
	...
	public void AddRemoteConnection_rejects_a_relative_endpoint_at_registration() {
		...
		var act = () => builder.AddRemoteConnection<TestConnection>(
			options => options.EndpointUri = new Uri("/hubs/chat", UriKind.Relative));
	...
	public void Registering_the_same_type_with_different_options_throws() {
		...
		var act = () => builder.AddRemoteConnection<TestConnection>(Endpoint("https://api.example.com/hubs/admin"));
	...
	public void Different_connection_types_register_independently() {
		...
		var builder = new TestBuilder();
		builder.AddRemoteConnection<TestConnection>(Endpoint(HubUri));
		builder.AddRemoteConnection<OtherConnection>(Endpoint("https://api.example.com/hubs/admin"));
	...
	public async Task A_per_instance_adjustment_does_not_reach_later_instances() {
		...
		factory.Create(options => options.EndpointUri = new Uri("https://api.example.com/hubs/session"));
	...
	public void Registering_the_same_factory_type_with_different_options_throws() {
		...
		var act = () => builder.AddRemoteConnectionFactory<TestConnection>(Endpoint("https://api.example.com/hubs/admin"));
